...
### Pipeline- [View Pipeline Notebook](Pipeline/PipelineProject2 (1).ipynb)  
### Pipeline (Markdown)- [View Markdown Version](Pipeline/PipelineProject2 (1).md)
...
Several steps were taken, or can be taken, to reduce the impact of bias in the analysis. To address the issue of repeated patient encounters, the analysis focuses on encounter-level predictions rather than making claims about individual patients. Additionally, variables such as prior inpatient, outpatient, and emergency visits are included to help account for patient history and partially control for the overrepresentation of frequently admitted patients.

To deal with confounding from underlying health severity, control variables such as age, number of diagnoses, and prior hospital visits are included in the model. These variables act as proxies for patient health status, helping to separate the effect of treatment complexity from the effect of how sick a patient is. While this does not completely eliminate confounding, it reduces the likelihood that the results are driven purely by underlying severity rather than treatment patterns.

For the loss of detail caused by simplifying medication variables, the analysis retains both the aggregated treatment complexity measure and the individual medication indicators. This allows for comparison between the overall complexity score and specific medications, helping to identify whether results are driven by particular drugs rather than the simplified encoding. Although the “Up” and “Down” distinctions are not fully preserved, keeping individual medication variables helps maintain some level of interpretability.

Finally, to address potential sampling bias, random sampling was used to create the reduced dataset. This helps preserve the overall distribution of the data. In addition, summary statistics (such as readmission rates and medication frequencies) can be compared between the full dataset and the sample to check whether the sample remains representative. If large differences are observed, the sampling approach can be adjusted.
